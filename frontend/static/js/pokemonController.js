...
const PokemonController = {
    ...
    async startGame() {
        try {
            ...
            const startLoc = PokemonRenderer.getLocationCenter(this.state.startWeek);
    ...
    _gameLoop() {
        ...
        PokemonRenderer.updateCamera(this.state.playerX, this.state.playerY);
    ...
    _updatePlayer() {
        ...
        if (this.state.isMoving) {
            ...
            const nearest = PokemonRenderer.getNearestPathPoint(newX, newY);
            if (nearest && nearest.dist < PokemonRenderer.tileSize * 3) {
                const pull = Math.max(0, 1 - nearest.dist / (PokemonRenderer.tileSize * 3));
            ...
            this.state.playerX = Math.max(0, Math.min(this.state.playerX, PokemonRenderer.mapWidth * PokemonRenderer.tileSize));
            this.state.playerY = Math.max(0, Math.min(this.state.playerY, PokemonRenderer.mapHeight * PokemonRenderer.tileSize));
        }
        const nearWeek = PokemonRenderer.getLocationAt(this.state.playerX, this.state.playerY, PokemonRenderer.tileSize * 1.8);
